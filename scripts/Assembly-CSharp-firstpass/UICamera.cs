...
public partial class UICamera : MonoBehaviour
{
	...
	public class MouseOrTouch
	{
		...
		public float deltaTime
		{
			get
			{
				return (!touchBegan) ? 0f : (RealTime.time - pressTime);
			}
		}
	...
	public static Collider lastRaycastedCollider { get; private set; }

	public static Collider2D lastRaycastedCollider2D { get; private set; }
	...
	private IEnumerator ChangeSelection()
	{
		...
		if (onSelect != null)
		{
			onSelect(mCurrentSelection, false);
		}
	...
	{
		lastRaycastedCollider = null;
		lastRaycastedCollider2D = null;
		for (int i = 0; i < list.size; i++)
		{
			UICamera uICamera = list.buffer[i];
			...
			if (uICamera.eventType == EventType.World_3D)
			{
				...
				lastRaycastedCollider = lastHit.collider;
				return true;
			}
			if (uICamera.eventType == EventType.UI_3D)
			{
				...
				{
					if (raycastHit.transform.tag == "block")
					{
						return false;
					}
				}
				if (array.Length > 1)
				{
					for (int k = 0; k < array.Length; k++)
					{
						...
						if (component != null)
						{
							if (!component.isVisible || (component.hitCheck != null && !component.hitCheck(array[k].point)))
							{
								continue;
							}
						}
						...
						{
							mHit.hit = array[k];
							mHit.point = array[k].point;
							mHit.go = array[k].collider.gameObject;
							mHits.Add(mHit);
						}
					}
					mHits.Sort((DepthEntry r1, DepthEntry r2) => r2.depth.CompareTo(r1.depth));
					for (int l = 0; l < mHits.size; l++)
					{
						if (IsVisible(ref mHits.buffer[l]))
						{
							lastHit = mHits[l].hit;
							hoveredObject = mHits[l].go;
							lastWorldPosition = mHits[l].point;
							mHits.Clear();
							lastRaycastedCollider = mHits[l].hit.collider;
							return true;
						}
					}
					mHits.Clear();
				...
				else
				{
					...
					if (IsVisible(array[0].point, array[0].collider.gameObject))
					{
						...
						hoveredObject = lastHit.collider.gameObject;
						lastRaycastedCollider = array[0].collider;
						return true;
					}
				}
			}
			else
			{
				if (uICamera.eventType == EventType.World_2D)
				{
					...
					lastRaycastedCollider2D = collider2D;
					return true;
				}
				...
				Collider2D[] array3 = Physics2D.OverlapPointAll(lastWorldPosition, layerMask);
				if (array3.Length > 1)
				{
					...
					for (int n = 0; n < mHits.size; n++)
					{
						...
						{
							hoveredObject = mHits[n].go;
							mHits.Clear();
							lastRaycastedCollider = mHits[n].hit.collider;
							return true;
						}
					}
					mHits.Clear();
				...
				else
				{
					...
					if (IsVisible(lastWorldPosition, gameObject4))
					{
						...
						lastRaycastedCollider2D = array3[0];
						return true;
					}
				}
			}
	...
	public virtual void ProcessTouch(bool pressed, bool unpressed)
	{
		...
		if (currentTouch.pressed != null)
		{
			...
			if (currentTouch.dragged == currentTouch.current || (currentScheme != ControlScheme.Controller && currentTouch.clickNotification != 0 && currentTouch.totalDelta.sqrMagnitude < num))
			{
				if (currentTouch.pressed != mCurrentSelection)
				{
					...
					if (onSelect != null)
					{
						onSelect(currentTouch.pressed, true);
					}
