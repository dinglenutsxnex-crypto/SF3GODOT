...
public abstract partial class ServerProviderBase : MonoBehaviour
{
	...
	public class Row
	{
		...
		public Value this[string index]
		{
			get
			{
				...
				{
					Debug.LogWarning("there no data for key " + index.ToLower());
				}
	...
	protected static T Init<T>() where T : ServerProviderBase
	{
		...
		if (!_nestedObject)
		{
			...
			StaticObjectsManager.AddObject(_nestedObject, false);
		}
	...
	protected bool Check()
	{
		...
		if (!_nestedObject)
		{
			Debug.LogError("ServerCall terminated. you should call Init<T>() from your Init() method! (where T is your inherited class)");
			return false;
		}
		return true;
	}
	...
	public virtual void Join(Action<bool> onDone, Action<string> onError)
	{
		if (Check())
		{
			...
			{
				Debug.LogWarning("ServerProvider.Join(..) hold while SocialWrapper.CurrentUser is null");
	...
	protected virtual IEnumerator JoinRoutine(Action<bool> onDone, Action<string> onError)
	{
		...
		try
		{
			ServerResponce serverResponce = ServerResponce.Get(www.text);
			if (string.IsNullOrEmpty(serverResponce.error))
			{
				onDone(serverResponce.response.Equals("new"));
			}
			else
			{
				onError(serverResponce.error ?? string.Empty);
			}
		}
		catch (Exception ex)
		{
	...
	public virtual void SaveData(string alias, string data, Action onDone, Action<string> onError)
	{
		if (Check())
		{
			...
			if (!Social.CurrentUser)
			{
				Debug.LogWarning("ServerProvider.SaveData(..) hold while SocialWrapper.CurrentUser is null");
				_holdRoutine.Add(enumerator);
			}
			else
			{
	...
	public virtual void LoadData(string alias, Action<string> onDone, Action<string> onError)
	{
		if (Check())
		{
			...
			if (!Social.CurrentUser)
			{
				Debug.LogWarning("ServerProvider.LoadData(..) holden while SocialWrapper.CurrentUser is null");
				_holdRoutine.Add(enumerator);
			}
			else
			{
	...
	protected virtual IEnumerator LoadDataRoutine(string alias, Action<string> onDone, Action<string> onError)
	{
		...
		try
		{
			...
			if (string.IsNullOrEmpty(serverResponce.error))
			{
				onDone(Unescape(serverResponce.response));
			}
			else
			{
				onError(serverResponce.error ?? string.Empty);
	...
	{
		WWW www = new WWW(form: new Form(), url: string.Format("{0}/time.php", GetServer()));
		yield return www;
		if (!string.IsNullOrEmpty(www.error))
		{
			onError(www.error);
		...
		try
		{
			...
			if (string.IsNullOrEmpty(serverResponce.error))
			{
				onDone(long.Parse(serverResponce.response.Trim()));
			}
			else
			{
				onError(serverResponce.error ?? string.Empty);
	...
	public virtual void WipeUser(Action onDone, Action<string> onError)
	{
		if (Check())
		{
			...
			if (!Social.CurrentUser)
			{
				Debug.LogWarning("ServerProvider.WipeUser(..) hold while SocialWrapper.CurrentUser is null");
				_holdRoutine.Add(enumerator);
			}
			else
			{
	...
	protected virtual IEnumerator QueryRoutine(Select select, Action<QResponse> onDone, Action<string> onError)
	{
		...
		try
		{
			...
			if (string.IsNullOrEmpty(jSONNode["error"]))
			{
				onDone(new QResponse(jSONNode["response"].AsArray, select));
			}
			else
			{
