...
public partial class UIRoot : MonoBehaviour
{
	...
	public void OnApplicationFocus(bool focus)
	{
		if (focus)
		{
			StartCoroutine(Reactivale());
		}
	}

	private IEnumerator Reactivale()
	{
		for (int i = 0; i < 3; i++)
		{
			UIWidget[] widgets = base.gameObject.GetComponentsInChildren<UIWidget>();
			if (widgets != null)
			{
				UIWidget[] array = widgets;
				foreach (UIWidget uIWidget in array)
				{
					if (uIWidget is UILabel || uIWidget is UISprite || uIWidget is UITexture)
					{
						uIWidget.SendMessage("OnEnable", SendMessageOptions.DontRequireReceiver);
					}
				}
			}
