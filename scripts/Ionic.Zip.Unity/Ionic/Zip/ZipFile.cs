...
namespace Ionic.Zip
{
	...
	public class ZipFile : IEnumerable<ZipEntry>, IEnumerable, IDisposable
	{
		...
		internal bool Verbose
		{
			get
			{
				return _StatusMessageTextWriter != null;
			}
		}
		...
		public int NumberOfSegmentsForMostRecentSave
		{
			get
			{
				return (int)(_numberOfSegmentsForMostRecentSave + 1);
			}
		}
		...
		public ICollection<ZipEntry> EntriesSorted
		{
			get
			{
				...
				bool caseSensitiveRetrieval = CaseSensitiveRetrieval;
		...
		public void Save()
		{
			try
			{
				...
				_numberOfSegmentsForMostRecentSave = ((zipSegmentedStream == null) ? 1u : zipSegmentedStream.CurrentSegment);
				...
				if (_name != null && (_temporaryFileName != null || zipSegmentedStream != null))
				{
					...
					if (_fileAlreadyExists && _readstream != null)
					{
						...
						{
							ZipSegmentedStream zipSegmentedStream2 = item2._archiveStream as ZipSegmentedStream;
		...
		{
			// Self-extracting EXE generation used System.CodeDom.Compiler (CSharpCodeProvider) to compile a
			// Windows-only stub executable at runtime. Not supported/needed outside desktop Unity tooling;
			// stubbed out during the Godot port.
			throw new NotSupportedException("Self-extracting archive generation is not supported in this build.");
		}
